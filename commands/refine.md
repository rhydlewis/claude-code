...
/refine [optional: brief idea description]
...
- **Epic**: Large initiative needing decomposition (suggest `/split`)
...
Ask for confirmation via AskUserQuestion, then create:
```bash
bd create \
  --title="[title]" \
  --type=[type] \
...
Capture and present the new bead ID.

### Phase 9: Next Steps

Suggest appropriate next actions:
- If complex/uncertain: "Consider `/split` to break it down" or "Create a spike first"
- If ready: "Use `/go` to start working" or "Run `bd ready` to check blockers"
- If dependencies: "Create dependency beads and link with `bd dep add`"
...
- ✅ Keep scope tight (use `/split` for large work)
...
8. **Create**: Generate bead with `bd create`
9. **Next**: Suggest next steps (split, spike, start work)
